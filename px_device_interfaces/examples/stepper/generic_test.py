"""Generic stepper driver smoke test (step/dir/enable, no microstepping IC).

Usage::

    # Real hardware
    python -m px_device_interfaces.examples.stepper.generic_test --port /dev/ttyACM0

    # Offline (no hardware needed)
    python -m px_device_interfaces.examples.stepper.generic_test --mock
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generic stepper smoke test")
    ...
    try:
        ...
        stepper.configure_motion_rev(
            steps_per_rev=200,
            max_speed_rpm=30.0,
            max_accel_rpm_s=30.0,
        )
        stepper.set_current_position_rev(0.0)

        print("Moving +0.25 rev …")
        stepper.move_to_position_rev(0.25, speed_override_rpm=20.0, accel_override_rpm_s=20.0)
        _poll_until_stopped(stepper)

        print("Moving back to origin …")
        stepper.move_to_position_rev(0.0, speed_override_rpm=20.0, accel_override_rpm_s=20.0)
        _poll_until_stopped(stepper)

        print("Decelerating stop test …")
        stepper.move_to_position_rev(0.75, speed_override_rpm=24.0, accel_override_rpm_s=24.0)
        time.sleep(0.4)
