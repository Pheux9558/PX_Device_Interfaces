    ...
    def _recv_worker(self) -> None:
        while self._running:
            try:
                ...
                data = None
                # Guard against `self._transport` being None to satisfy type checkers
                if self._transport is not None and hasattr(self._transport, "receive_bytes"):
                    data = self._transport.receive_bytes(timeout=0.5)
                if data is None:
                    # fallback to text-mode receive
                    if self._transport is None:
                        # no transport available; wait briefly and continue
                        time.sleep(0.001)
                        continue
